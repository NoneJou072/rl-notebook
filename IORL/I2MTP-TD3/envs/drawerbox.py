...
class DrawerBox(DrawerCubeEnv):
    ...
    def _get_obs(self) -> dict:
        ...
        obs[21:24] = (  # handle velocity with respect to the gripper
            handle_velp - end_vel
        )
        block_velp = self.get_body_xvelp('green_block') * dt
        obs[24:27] = (  # block velocity with respect to the gripper
            block_velp - end_vel
        )
    ...
    def _get_desired_goal(self):
        desired_goal = np.concatenate([
            self.get_site_pos('drawer') if self._is_success(
                self.get_site_pos('drawer'), self.get_site_pos('drawer_goal'), th=0.02
    ...
    def _get_info(self) -> dict:
        return {
            'is_drawer_success': self._is_success(self.get_site_pos('drawer'), self.get_site_pos('drawer_goal'), th=0.02),
