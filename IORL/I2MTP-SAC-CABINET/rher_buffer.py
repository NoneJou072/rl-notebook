...
class RHERReplayBuffer(ReplayBuffer):
    ...
    def sample(self, batch_size: int = 256, sequential: bool = True, with_log=True, device='cpu', task='reach'):
        ...
        :param task: (str) reach, unlock, door
        ...
        if task == 'reach':
            ...
            rewards = np.expand_dims(self.env.compute_reward(achieved_goals[:, 3:6], desired_goals[:, 3:6], th=0.05), 1)
            desired_goals[:, :3] *= 0
            desired_goals[:, 6:] *= 0
        elif task == 'door':
            future_ag = []
            ...
            rewards = np.expand_dims(self.env.compute_reward(achieved_goals[:, 6:], desired_goals[:, 6:], th=0.03), 1)
            desired_goals[:, :6] *= 0
