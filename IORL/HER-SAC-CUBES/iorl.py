...
class IORL:
    def __init__(self, env, args):
        ...
        self.memory_red = RHERReplayBuffer(capacity=args.buffer_size, k_future=args.k_future, env=env)
        self.memory_green = RHERReplayBuffer(capacity=args.buffer_size, k_future=args.k_future, env=env)
        self.memory_blue = RHERReplayBuffer(capacity=args.buffer_size, k_future=args.k_future, env=env)
    ...
    def sample_action(self, s, task, deterministic=False):
        assert task in ['red', 'green', 'blue', 'reach']

        last_obs = copy.deepcopy(s)
        reached = self.check_reached(last_obs['achieved_goal'][:3], last_obs['desired_goal'][:3], th=0.02)
    ...
    def update(self):
        ...
        batch_s_4, batch_a_4, batch_s_4_, batch_r_4, batch_g_4 = self.memory_blue.sample(self.batch_size,
                                                                                         device=self.device,
                                                                                         task='blue')
        batch_s_3, batch_a_3, batch_s_3_, batch_r_3, batch_g_3 = self.memory_green.sample(self.batch_size,
                                                                                         device=self.device,
                                                                                         task='green')
        batch_s_2, batch_a_2, batch_s_2_, batch_r_2, batch_g_2 = self.memory_red.sample(self.batch_size,
                                                                                         device=self.device,
                                                                                         task='red')
        batch_s_1, batch_a_1, batch_s_1_, batch_r_1, batch_g_1 = self.memory_reach.sample(self.batch_size,
                                                                                          device=self.device,
                                                                                          task='reach')
        batch_s = torch.concatenate((batch_s_1, batch_s_2, batch_s_3, batch_s_4))
        batch_a = torch.concatenate((batch_a_1, batch_a_2, batch_a_3, batch_a_4))
        batch_s_ = torch.concatenate((batch_s_1_, batch_s_2_, batch_s_3_, batch_s_4_))
        batch_r = torch.concatenate((batch_r_1, batch_r_2, batch_r_3, batch_r_4))
        batch_g = torch.concatenate((batch_g_1, batch_g_2, batch_g_3, batch_g_4))
