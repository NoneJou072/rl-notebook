...
def train(args):
    ...
    # Tensorboard config
    log_dir = os.path.join(log_path, f'./runs/GAIL-SAC')
    ...
    with tqdm(total=args.n_epochs) as pbar:
        ...
        for epoch in range(args.n_epochs):
            ...
            if args.use_state_norm:
                ...
                rollout_success = roll_out(args, generator, env, 1)
            ...
            for ep_step in range(args.max_episode_horizon):
                action = generator.sample_action(obs["observation"])
                ...
                generator.memory.push((
                    obs["observation"], action, next_obs["observation"], irl_reward, termination, truncation
                ))
            ...
            if epoch % args.save_freq == 0: 
                ...
                torch.save(generator.actor.state_dict(), os.path.join(model_dir, f'GAILSAC_{epoch}.pth'))
                if args.use_state_norm:
                    state_norm.save(os.path.join(model_dir, f'GAILSAC_state_norm_{epoch}.npy'))
                print("Model saved!")
...
if __name__ == '__main__':
    ...
    # Training Params
    args.add_argument("--n_epochs", type=int, default=int(2e5), help=" Maximum number of training steps")
    args.add_argument("--n_episodes", type=int, default=1, help=" Maximum number of training steps")
    args.add_argument("--max_train_steps", type=int, default=int(2e7), help=" Maximum number of training steps")
    args.add_argument("--max_episode_horizon", type=int, default=int(500), help=" Maximum number of training steps")
    args.add_argument("--save_freq", type=int, default=100, help="Save frequency")
    args.add_argument("--buffer_size", type=int, default=int(1e6), help="Reply buffer size")
    args.add_argument("--batch_size", type=int, default=256, help="Minibatch size")
    # Net Params
    ...
    args.add_argument("--tau", type=float, default=0.005, help="Softly update the target network")
    ...
    args.add_argument("--use_state_norm", type=bool, default=False, help="Trick 2:state normalization")
    args.add_argument("--use_lr_decay", type=bool, default=False, help="Trick 6:learning rate Decay")
    args.add_argument("--use_orthogonal_init", type=bool, default=False, help="Trick 8: orthogonal initialization")
